<?php
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST" && !empty($_POST['test_id']) && !empty($_POST['user_id'])) {
        ...
        if (!$examinee) {
            ...
            require_once('utils.php');
            redirectToPageWithPost('https://eso.vse.cz/~holo00/etester/test_properties.php?test_id=' . htmlspecialchars($_POST['test_id']), 'Wrong operation. User does not have the examinee role.', 'alert-danger');
            exit();
        } elseif ($testUser) {
            require_once('utils.php');
            redirectToPageWithPost('https://eso.vse.cz/~holo00/etester/test_properties.php?test_id=' . htmlspecialchars($_POST['test_id']), 'The examinee already has access.', 'alert-warning');
            exit();
        } else {
            ...
            require_once('utils.php');
            redirectToPageWithPost('https://eso.vse.cz/~holo00/etester/test_properties.php?test_id=' . htmlspecialchars($_POST['test_id']), 'User ' . @$examinee['email'] . ' can now access the test.', 'alert-success');
            exit();
        }
    } else {
        require_once('utils.php');
