<?php
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST" && !empty($_POST['test_id']) && !empty($_POST['user_id'])) {
        ...
        if (!$examinee) {
            ...
            require_once('inc/utils.php');
            redirectToPageWithPost('test_properties.php?test_id=' . htmlspecialchars($_POST['test_id']), 'User access to the test has been revoked.  (' . @$examinee['email'] . ')', 'alert-success');
            exit();
        } else {
            require_once('inc/utils.php');
            redirectToPageWithPost('test_properties.php?test_id=' . htmlspecialchars($_POST['test_id']), 'The examinee already did not have access.', 'alert-warning');
            exit();
        }
    } else {
        require_once('inc/utils.php');
